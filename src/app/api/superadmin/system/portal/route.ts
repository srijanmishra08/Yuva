...
export async function GET(req: NextRequest) {
  ...
  const { data } = await supabase.from('portal_config').select('config_value').eq('config_key', 'portal').single();
...
export async function PUT(req: NextRequest) {
  ...
  const supabase = createAdminClient();
  await supabase.from('portal_config').update({ config_value: body, updated_by: auth.userId }).eq('config_key', 'portal');
